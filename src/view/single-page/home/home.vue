  ...
  <div>
    ...
    <Row>
      <Card shadow style="width:100%">
        <week-state :key="timer4" :weekData="weekData" style=" height: 310px"></week-state>
      </Card>
    </Row>
  </div>
</template>
...
import WeekState from './weekstat.vue'
    ...
    getData () {
      getStatData().then((res) => {
        if (res.code === 200) {
          ...
          //   console.log('TCl: -> this.inforCardData.forEach -> index', index)
          //   console.log('TCl: -> this.inforCardData.forEach -> item', item)
        ...
        console.log('TCl: -> getStatData -> res', res)
      })
    }
  }
}
